...
static NSString * const kCustomEvent = @"hello";
...
- (void)viewDidLoad
{
    ...
    /* FIXME:
     ...
     该 token 不 应该保存在客户端中，所有涉及该 token 的请求，应该放在服务器中。
     */
    self.sdkToken = @"";
    self.view.backgroundColor = [UIColor orangeColor];
    //self.roomUuid = @"3dc7ef1c7ed446e082c1ebf3973d83f4";
...
- (void)joinRoom
{
    ...
    [self getRoomTokenWithUuid:self.roomUuid Result:^(BOOL success, id response, NSError *error) {
        if (success) {
            NSString *roomToken = response[@"msg"][@"roomToken"];
            NSLog(@"uuid=%@,rootoken=%@",self.roomUuid,roomToken);
            [self joinRoomWithToken:roomToken];
...
- (void)fireMagixEvent:(WhiteEvent *)event
{
    ...
    NSInteger type = [[event.payload[@"type"] description] integerValue];
    [self.eventDelegate fireMagixEvent:type];
