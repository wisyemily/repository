...
- (void)layoutVideoViews {
    ...
    if (self.switchType == 0||self.switchType == 4||self.switchType == 2) {
    ...
    } else if (self.switchType == 5||self.switchType == 1||self.switchType == 6||self.switchType == 7) {
        if (self.fullView)
        {
            UIView *peerView = self.videoViews.firstObject;
            NSArray *cornerViewLayouts = [self layoutFullSessionView:peerView inContainerView:self.zoomView];
            [self.layoutConstraints addObjectsFromArray:cornerViewLayouts];
        }
        else
        {
            UIView *peerView = self.videoViews.firstObject;
            NSArray *fullViewLayouts = [self layoutLeftCornerSessionView:self.selfView inContainerView:self.containerView];
            [self.layoutConstraints addObjectsFromArray:fullViewLayouts];
            
            NSArray *cornerViewLayouts = [self layoutRightCornerSessionView:peerView inContainerView:self.containerView];
            [self.layoutConstraints addObjectsFromArray:cornerViewLayouts];
        }
        /*NSMutableArray *layouts = [[NSMutableArray alloc] init];
        UIView *firstView = self.selfView;
        UIView *lastView = self.videoViews.firstObject;
        [self.containerView addSubview:firstView];
        [self.containerView addSubview:lastView];
        ...
        NSArray<NSLayoutConstraint *> *v1 = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|[view1]|" options:0 metrics:nil views:@{@"view1": firstView}];
        NSArray<NSLayoutConstraint *> *v2 = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|[view1]|" options:0 metrics:nil views:@{ @"view1": lastView}];
        NSLayoutConstraint *equal = [NSLayoutConstraint constraintWithItem:firstView attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:lastView attribute:NSLayoutAttributeRight multiplier:1 constant:0];
        ...
        [self.layoutConstraints addObjectsFromArray:[layouts copy]];*/
...
- (NSArray<NSLayoutConstraint *> *)layoutFullSessionView:(UIView *)view inContainerView:(UIView *)container {
   ...
   // view.frame = container.frame;
    NSLog(@"w=%f,h=%f",container.frame.size.width,container.frame.size.height);
    NSLog(@"view w=%f,h=%f",view.frame.size.width,view.frame.size.height);
...
- (NSArray<NSLayoutConstraint *> *)layoutLeftCornerSessionView:(UIView *)view inContainerView:(UIView *)container {
    NSMutableArray *layouts = [[NSMutableArray alloc] init];
    [container addSubview:view];
    
    NSLayoutConstraint *right = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:container attribute:NSLayoutAttributeLeft multiplier:1 constant:0];
    NSLayoutConstraint *top = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:container attribute:NSLayoutAttributeTop multiplier:1 constant:0];
    NSLayoutConstraint *width = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1 constant:container.frame.size.width/2];
    NSLayoutConstraint *height = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1 constant:container.frame.size.height];
...
- (NSArray<NSLayoutConstraint *> *)layoutRightCornerSessionView:(UIView *)view inContainerView:(UIView *)container {
    NSMutableArray *layouts = [[NSMutableArray alloc] init];
    [container addSubview:view];
    
    NSLayoutConstraint *right = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:container attribute:NSLayoutAttributeRight multiplier:1 constant:0];
    NSLayoutConstraint *top = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:container attribute:NSLayoutAttributeTop multiplier:1 constant:0];
    NSLayoutConstraint *width = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1 constant:container.frame.size.width/2];
    NSLayoutConstraint *height = [NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1 constant:container.frame.size.height];
