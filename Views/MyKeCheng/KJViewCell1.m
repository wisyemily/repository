...
@implementation KJViewCell1

+ (KJViewCell1*)cell {
    return [[NSBundle mainBundle] loadNibNamed:@"KJViewCell1" owner:nil options:nil].lastObject;
}
...
- (IBAction)enterAction:(id)sender {
    if (self.action)
        self.action(self, KJActionType1Enter);
}

- (IBAction)deleteAction:(id)sender {
    if (self.action)
        self.action(self, KJActionType1Bind);
}
